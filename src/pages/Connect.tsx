...
export function Connect() {
  const { setConnecting, isConnecting, setConnected, isConnected, setReading, isReading, offlineDevice, offlineFirmware, setOfflineDevice, setOfflineFirmware} = useGlobal();
  ...
  function download_selected_map(){
    if (!offlineDevice || !offlineFirmware) {
      toast.error(lang === "pt-br" ? "Selecione um dispositivo e um firmware antes de baixar o mapa." : "Select a device and firmware before downloading the map.");
      return;
    }

    invoke("donwload_client_csv", {
      device: offlineDevice,
      firmware: offlineFirmware,
      lang: lang.startsWith("pt") ? "pt" : "en",
    }).then(() => {
      toast.success(lang === "pt-br" ? "Download concluído" : "Download complete");
    }).catch((err) => {
      toast.error(lang === "pt-br" ? "Falha ao baixar o mapa" : "Failed to download the map");
      console.error(err);
    });
  }
  ...
  return (
    <div className="flex flex-col items-center w-full justify-center h-full">
      ...
      <section className="w-fit flex flex-col justify-center gap-4">
        ...
        {/* IED e Firmware e download do mapa */}
        <div className="flex  gap-4 justify-center ">
          {/*! IED !*/}
          <DropdownMenu >

            <DropdownMenuTrigger disabled={isConnecting || isReading || isConnected} asChild className="flex-1 min-w-0 bg-card p-4 text-lg hover:border-primary">
          ...
          {/*! Firmware !*/}
          <DropdownMenu>
            <DropdownMenuTrigger disabled={isConnecting || isReading || isConnected} asChild className="flex-1 min-w-0 bg-card text-lg p-4 hover:border-primary">
              <InputGroupButton variant="ghost" className="cursor-pointer">
          ...
          <Button className={`${!connection.device ? "hidden" : ""}`} onClick={download_selected_map}>Download Map</Button>
