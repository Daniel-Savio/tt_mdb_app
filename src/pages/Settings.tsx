...
export function Settings() {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      //Com comunicação
        ...
        let raw_data: string = await invoke("start_reading");
        let jsonData = JSON.parse(raw_data) as RawJsonReading[];
        return jsonData;
      }
    },
  ...
  useEffect(() => {
    const initializeValue = async () => {
      ...
      if (isConnected) {
        ...
        try {
          ...
          if (val !== null) {
            ...
            if (isSelect) {
              const options =
                selectedReading["Conversão pt"]?.split("\\") || [];
      ...
      if (isSelect) {
        ...
        initial =
          options[idx]?.trim() || selectedReading["Valor default"] || "";
      } else {
  ...
  useEffect(() => {
    const unlistenStop = listen("reading-stop", () => {
      ...
      setConnected(false);
      toast.error(
        lang === "pt-br" ? "Comunicação perdida" : "Communication lost",
      );
    });
  ...
  const isDefault = useMemo(() => {
    ...
    // Se estivermos online e tivermos o valor lido no objeto, usamos ele como referência principal
    if (
      isConnected &&
      ...
      if (isSelect) {
        const options = selectedReading["Conversão pt"]?.split("\\") || [];
        const idx = Math.round(selectedReading.value);
        const readStr =
          options[idx]?.trim() || selectedReading.value.toString();
        return currentValue === readStr;
      }

      const currentNum = parseFloat(currentValue || "0");
      const readDivided = selectedReading.value / divisor;
      return Math.abs(currentNum - readDivided) < 0.00001; // Usando margem para float
    }
    ...
    return Math.abs(currentNum - dividedDefault) < 0.00001;
  }, [currentValue, selectedReading, isConnected]);

  // Função centralizada para salvar automaticamente
  const autoSaveToDrawer = useCallback(
    (value: string) => {
      if (!selectedReading) return;
  ...
  const exportAllToJson = async () => {
    ...
    const exportData = data.map((point) => {
      ...
      if (modifiedPoints[point.UUID]) {
        ...
        if (isConnected && point.value !== undefined && point.value !== null) {
          if (isSelect) {
            const options = point["Conversão pt"]?.split("\\") || [];
            const idx = Math.round(point.value);
            value = options[idx]?.trim() || point.value.toString();
          } else {
            value = (point.value / divisor).toString();
          }
        } else {
          if (isSelect) {
            const options = point["Conversão pt"]?.split("\\") || [];
      ...
      return {
        ...
        Valor: value,
        Descrição:
          lang === "pt-br" ? point["Descrição pt"] : point["Descrição en"],
      };
    });
    ...
    try {
      ...
      console.error(err);
      toast.error(
        lang === "pt-br" ? "Erro ao exportar arquivo" : "Error exporting file",
      );
    }
  };
  ...
  const importFromJson = async () => {
    try {
      ...
      const path =
        typeof filePath === "object" ? (filePath as any).path : filePath;
      ...
      if (!Array.isArray(importedData)) {
        toast.error(
          lang === "pt-br"
            ? "Formato de arquivo inválido: Esperado um Array"
            : "Invalid file format: Array expected",
        );
        return;
      }
      ...
      importedData.forEach((importedPoint: any) => {
        ...
        const importedValue =
          importedPoint["Valor"] ??
          importedPoint["Valor default"] ??
          importedPoint["newValue"];
        ...
        if (
          ...
          // Calcula o valor de referência (do equipamento ou fábrica) no formato humano
          if (
            isConnected &&
            ...
            if (isSelect) {
              ...
              const idx = Math.round(currentPoint.value);
              referenceValue =
                options[idx]?.trim() || currentPoint.value.toString();
            } else {
              referenceValue = (currentPoint.value / divisor).toString();
            }
          } else {
            if (isSelect) {
              const options = currentPoint["Conversão pt"]?.split("\\") || [];
              ...
              referenceValue =
                options[idx]?.trim() || currentPoint["Valor default"] || "";
            } else {
              referenceValue = (
                parseFloat(currentPoint["Valor default"] || "0") / divisor
              ).toString();
            }
          }
          ...
          // Só importa se o valor for diferente da referência atual
          if (valStr !== referenceValue) {
    ...
    for (const point of points) {
      ...
      try {
        ...
        if (point.original["Conversão pt"]) {
          const options = point.original["Conversão pt"]
            .split("\\")
            .map((o) => o.trim());
  ...
  return (
    <section className="flex flex-col w-full h-full p-4 relative overflow-hidden">
      <header className="flex flex-row items-center justify-between w-full mb-4">
        <div className="flex items-center gap-4">
          {isConnected ? (
            <h1 className="text-xl font-bold">
              {lang === "pt-br" ? "Parâmetros:" : "Settings:"}{" "}
              {connection.device} - {connection.firmware}
            </h1>
          ) : (
            <h1 className="text-xl font-bold">
              {lang === "pt-br" ? "Parâmetros:" : "Settings:"} {offlineDevice} -{" "}
              {connection.firmware}
            </h1>
          )}
          ...
          <div className="flex items-center gap-2 border-l pl-4 ml-2">
            ...
            >
              <RefreshCcw
                className={`h-4 w-4 ${isRefetching ? "animate-loading" : ""}`}
              />
              {isReadingValue
                ? lang === "pt-br"
                  ? "Recarregando..."
        ...
        <span
          className={`px-2 py-1 rounded text-xs font-bold uppercase ${isConnected ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
        >
          ...
          {selectedReading ? (
            <div className="space-y-4" key={selectedReading.UUID}>
              <section className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {lang === "pt-br"
                      ? selectedReading["Descrição pt"]
                      : selectedReading["Descrição en"]}
                  </p>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">
                    {lang === "pt-br" ? "Caminho Display" : "Display Path"}
                  </label>
                  <p className="text-sm italic text-primary">
                    {selectedReading["Display pt"]}
                  </p>
                </div>
                <div>
                  <label className="text-xs  text-muted-foreground">
                    ...
                    !selectedReading["Conversão pt"] ? (
                      <p className="text-sm italic text-blue-400">
                        {(
                          ...
                          Number(selectedReading["Divisor"] || 1)
                        ).toString()}
                      </p>
                    ) : (
                      <p className="text-sm italic text-blue-400">
                        {selectedReading["Conversão pt"]
                          ?.split("\\")
                          [Math.round(selectedReading.value)]?.trim() ||
                          selectedReading.value.toString()}
                      </p>
                    )
                  ) : !selectedReading["Conversão pt"] ? (
                    <p className="text-sm italic text-blue-400">
                      {Number(selectedReading["Valor default"]) /
                        Number(selectedReading["Divisor"] || 1) || ""}
                    </p>
                  ) : (
                    <p className="text-sm italic text-blue-400">
                      {selectedReading["Valor default"]}
                    </p>
                  )}
                </div>
              </section>
              ...
              <div className="grid grid-cols-4 gap-2">
                <div>
                  <label className="text-xs text-muted-foreground">
                    {lang === "pt-br" ? "Registrador" : "Register"}
                  </label>
                  <p className="text-sm font-mono">
                    {selectedReading["Registrador (Modbus)"]}
                  </p>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">
                    {lang === "pt-br" ? "Tratamento/Bit" : "Treatment/Bit"}
                  </label>
                  <p className="text-sm font-mono">
                    {selectedReading["Tratamento"]}
                  </p>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">Type</label>
                  <p className="text-sm font-mono">
                    {selectedReading["Tipo (Modbus)"]}
                  </p>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">
                    {lang === "pt-br"
                      ...
                      : "Optional of the Equipment"}
                  </label>
                  <p className="text-sm font-mono">
                    {selectedReading["Opcional"]
                      ? selectedReading["Opcional"]
                      : "Default"}
                  </p>
                </div>
              ...
              <div className="flex flex-col gap-4 justify-center text-center items-center mt-5">
                <h1 className="text-lg font-bold">
                  {lang === "pt-br" ? "Valor" : "Value"} /{" "}
                  {selectedReading["Limite inferior"] ? (
                    <span className="text-xs text-muted-foreground">
                      {Number(selectedReading["Limite inferior"]) /
                        Number(selectedReading["Divisor"] || 1)}{" "}
                      -{" "}
                      {Number(selectedReading["Limite superior"]) /
                        Number(selectedReading["Divisor"] || 1)}
                    </span>
                  ) : (
                    <span className="text-xs text-muted-foreground">
                      {lang === "pt-br" ? "Nenhum limite" : "No limits"}
                    </span>
                  )}
                </h1>
                  ...
                  <>
                    {selectedReading["Conversão pt"] ? (
                      <div className="flex flex-col gap-1">
                        ...
                        >
                          <SelectTrigger
                            className={` w-60 flex gap-4 text-sm cursor-pointer transition-colors ${isDefault ? "border-blue-400 text-blue-400" : ""}`}
                          >
                ...
                {isDefault && (
                  <span className="text-sm font-medium  animate-in fade-in slide-in-from-top-1">
                    {isConnected &&
                    selectedReading.value !== undefined &&
                    selectedReading.value !== null ? (
                      <p className="flex items-center gap-1">
                        <SquareActivity />{" "}
                        {lang === "pt-br"
                          ? "* Valor igual ao lido do equipamento"
                          : "* Value matches equipment reading"}
                      </p>
                    ) : (
                      <p className="flex items-center gap-1">
                        <FactoryIcon />{" "}
                ...
                {modifiedPoints[selectedReading.UUID] && (
                  <div className="text-sm font-medium text-amber-400 animate-in fade-in slide-in-from-top-1">
            ...
            <div className="flex-1 flex items-center justify-center text-sm text-muted-foreground text-center">
              {lang === "pt-br"
                ? "Selecione um ponto na árvore para ver os detalhes"
                : "Select a point in the tree to view details"}
            </div>
          )}
        </div>
      </div>
